...
$ChainPort = $env:CHAIN_SERVER_PORT; if (-not $ChainPort) { $ChainPort = "6908" }
$CatalogPort = $env:CATALOG_RETRIEVER_PORT; if (-not $CatalogPort) { $CatalogPort = "6909" }
$MemoryPort = $env:MEMORY_RETRIEVER_PORT; if (-not $MemoryPort) { $MemoryPort = "6910" }
$RailsPort = $env:GUARDRAILS_PORT; if (-not $RailsPort) { $RailsPort = "6911" }
$MilvusPort = $env:MILVUS_PORT; if (-not $MilvusPort) { $MilvusPort = "6912" }
$MilvusHealthPort = $env:MILVUS_HEALTH_PORT; if (-not $MilvusHealthPort) { $MilvusHealthPort = "6913" }
$MinioPort = $env:MINIO_PORT; if (-not $MinioPort) { $MinioPort = "6914" }
$MinioConsolePort = $env:MINIO_CONSOLE_PORT; if (-not $MinioConsolePort) { $MinioConsolePort = "6915" }
$EtcdPort = $env:ETCD_PORT; if (-not $EtcdPort) { $EtcdPort = "6916" }
